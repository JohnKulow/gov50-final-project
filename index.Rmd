...
```{r}
joined_data |>
  select(Country, country, EFindex, year)
```
...
joined_data |>
  group_by(Country) |>
  summarize(num_cwar = sum(onset, na.rm = TRUE))
```
...
slaydata |>
  ...
  geom_bar() +
  labs(x = "Number of Civil Wars", y = "Number of Countries")
```

```{r}
slaydata |>
  ggplot(mapping = aes(x = new_ef, y = num_cwar)) +
  geom_point() +
  geom_smooth()
```

```{r}
fit_slay <- lm(num_cwar ~ new_ef, data = slaydata)
...
fit_slay2 <- lm(num_cwar ~ mean_efs, data = slaydata)
fit_slay2
...
fit_slay3 <- lm(num_cwar ~ mean_efs + secondeth, data = slaydata)
fit_slay3
...
efs_cwar <- civilwar |>
  ...
  summarize(years_num = n(),
            ...
            secondeth = mean(second),
            num_cwar = sum(onset),
            gdppc = mean(gdpen * onset, na.rm = TRUE),
            mts = mean(mtnest),
            ) |>
...
```{r, echo = FALSE}
fit2 <- lm(cwar_len ~ gdppc, data = efs_cwar_years)
fit2
...
cwar_play |>
  ggplot(mapping = aes(x = ef2, y = num_cwar)) +
  geom_point() +
  geom_smooth()
...
```{r}
fit_play <- lm(num_cwar ~ ef2 + years_num, data = cwar_play)
fit_play
